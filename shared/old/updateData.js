...
updateData.prototype.SaveInvoiceGetBackInvoiceID = function(opts, cb) {
  ...
  async.forEachLimit(opts.transactions,asyncLimit, function(v,  next) {
    ...
    if ( _.has(v, 'updateStatus') && v.updateStatus.status == false ) {
      process.send({ creatingTransaction: {transactionRef : v.ExternalReference, status : false, message: 'not a valid transaction so not created ' + v.updateStatus.error }});
	...
	this.aiq = new aiqClient(myClient, this.myConnection.pKey, this.myConnection.uKey, this.myConnection.coID);
        //var aiq = new aiqClient(client, opts.connection.pKey, opts.connection.uKey, opts.coID);
	      //console.log(v.EnvironmentIdentifier + ' ' +  v.PurchaseAccountID + ' this.aiq is ' + util.inspect(this.aiq, { showHidden: true, depth: null }));
        ...
        //  checkExternalReference = Q.all([aiq.GetInvoicesByExternalReference({externalReference: v.ExternalReference})]);
	//} else {
	//  r = {};
        //  r.Result = {};
        //  checkExternalReference = Q.all([aiq.GetInvoicesByExternalReference({externalReference: 'DESIGNED TO FAIL' })]);
        //  //checkExternalReference = Q.resolve(r.Result.Invoice = []); // wait for nothing,
       // }
       // checkExternalReference.then(([r]) => {
//	  console.log('GOT GetInvoiceByExternalReference RESULT' + JSON.stringify(r));
	  ...
	  console.log([templatingType] + JSON.stringify(v[accountID]));
          Q.all([this.aiq[transactionTemplate]({[templatingType]: v[accountID] })])
		    ...
		    if ( name != "uniqueExternalReferences" ) {
              ...
              Q.all([this.aiq[transactionType]({invoice: r1, create: true})])
                .then(([r2]) => {
                  console.log('got back from ' + transactionType + ' in updateData for ' + JSON.stringify(r1.ExternalReference) + ' result ' + JSON.stringify(r2));
                  ...
                  if ( r2.Status == "Created" ) {
                    ...
                    if ( v.Status == 'Posted' ) { // the invoice should be posted - cant do when save it, need to post it using the invoiceID
                      ...
                      Q.all([this.aiq['PostInvoiceGetBackTransactionID']({invoiceID: r2.invoiceID})])
                        .then(([r3]) => {
          	         console.log('got back from ' + transactionType + ' in updateData for ' + JSON.stringify(r1.ExternalReference) + ' result ' + JSON.stringify(r3));
          	       })
                       .fail(err => {
        ...
        //}) 
        //checkExternalReference.done(function() {
        //  next();
        //})
  ...
  async.forEachLimit(opts.transactions,asyncLimit, function(v,  next) {
	...
	this.aiq = new aiqClient(myClient, this.myConnection.pKey, this.myConnection.uKey, this.myConnection.coID);
          Q.all([this.aiq[transactionType]({ journal: v })])
            .then(([r2]) => {
              console.log('got back from ' + transactionType + ' in updateData for ' + JSON.stringify(v.ExternalReference) + ' result ' + JSON.stringify(r2));
...
// CreateGeneralJournal - it is very simple, has no "templating" to create a shell - so must come fully constructed (not that much to it!) 
updateData.prototype.SaveSalesReceiptGetBackTransactionID = function(opts, cb) {
  var envConnectionDetails = [];
  // push the "normal" or primary environment into the coID array 
  envConnectionDetails.push({ "coID": opts.coID, "value": "default", "uKey": opts.connection.uKey, "pKey": opts.connection.pKey, "url": opts.connection.url});
  // Multiple Environments ... if we have additionalEnv - add the details (change of coID and uKey plus a new value) into the array ...
  ...
  async.forEachLimit(opts.transactions,asyncLimit, function(v,  next) {
	...
	this.aiq = new aiqClient(myClient, this.myConnection.pKey, this.myConnection.uKey, this.myConnection.coID);
	  ...
	  if (v.hasOwnProperty('$$hashKey')) delete v.$$hashKey ;
	  if (v.hasOwnProperty('EnvironmentalIdentifier')) delete v.EnvironmentalIdentifier;
	  console.log('Create Receipt ' + JSON.stringify(v));
          Q.all([this.aiq[transactionType]({ 'salesReceipt': v })])
            .then(([r2]) => {
              console.log('got back from ' + transactionType + '  in updateData for ' + JSON.stringify(v.ExternalReference) + ' result ' + JSON.stringify(r2));
              if ( r2.Status == "Success" ) {
                console.log('updateData got ' + r2.Status);
		...
		// multi tier update - step one is the creation of the receipt, but then need to try to allocate it 
		multiStatus = { createdTransaction: {transactionRef : v.ExternalReference, status : true, message: 'transaction created - not attempted allocation', serverMessage: r2 }}
                // Now try to get Salesinvoice ID and allocate ...
	        Q.all([this.aiq.GetInvoicesByExternalReference({externalReference: v.ExternalReference})])
                  .then(([r3]) => {
		    console.log('GetInvoicesByExternalReference: ' + JSON.stringify(r3));
                    if (r3.Result) {
	              if ( r3.Result.Invoice.length > 0 ) {
	                console.log('We have found ' + r3.Result.Invoice.length + ' invoices with ExternalReference ' + v.ExternalReference ); 
	                console.log('We have found ' + r3.Result.Invoice[0].InvoiceNumber + ' invoice Number with ExternalReference ' + v.ExternalReference ); 
			// OK - so we have created the receipt and we now have an invoice to allocate against - so go ahead and allocate 
			// To allocate we need though to have the transaction ID's 
			myAllocation = {};
		        myAllocation.transactionID_1 =  r3.Result.Invoice[0].InvoiceNumber;
		        myAllocation.transactionID_2 =  r2.transactionID;
			myAllocation.allocationReference = v.ExternalReference;
			myAllocation.allocationAmount = v.PaymentAmount;
			myAllocation.allocationDate = v.PaymentDate;
                        console.log('GOING TO ALLOCATE ' + JSON.stringify(myAllocation));
	                Q.all([this.aiq.AllocateTransactions(myAllocation)])
                          .then(([r4]) => {
		            console.log('AllocationResult:' + JSON.stringify(r4));
		            multiStatus = multiStatus + r4;
                          })
                          .fail(err => {
		            console.log('AllocationResult Error:' + JSON.stringify(err));
		            multiStatus = multiStatus + err;
                          })
	                //return null;
	              } else {
	                console.log('No invoices with ExternalReference ' + v.ExternalReference ); 
		        multiStatus = { createdTransaction: {transactionRef : v.ExternalReference, status : true, message: 'transaction created - no transaction found to allocate to.', serverMessage: r3 }}
                        //process.send(multiStatus); 
	              }
	            } else {
		      console.log('GetInvoicesByExternalReference: has a no result');
		        multiStatus = { createdTransaction: {transactionRef : v.ExternalReference, status : true, message: 'transaction created - no transaction found to allocate to.', serverMessage: r3 }}
                        //process.send(multiStatus); 
		    }
                  })
                  .fail(err => {
		    multiStatus = { createdTransaction: {transactionRef : v.ExternalReference, status : true, message: 'transaction created - error in allocation ' + JSON.stringify(err) , serverMessage: r2 }}
                    process.send(multiStatus); 
                    //console.log('Error: in updateData ' + transactionType + ' :', errors[err.error])
                    // No r2 comes back 
                    //process.send({ createdTransaction: {transactionRef : v.ExternalReference, status : false, errorCode : 'SoapError' , message: err }}); 
                    console.log(err);
                  })
            ...
            .fail(err => {
              console.log('Error: in updateData ' + transactionType + ' :' + JSON.stringify(err) )
              // No r2 comes back 
              process.send({ createdTransaction: {transactionRef : v.ExternalReference, status : false, errorCode : 'SoapError Text' , message: multiStatus }}); 
              console.log(err);
            })
            .done(function() {
              console.log('COMPLETED UPDATE' + JSON.stringify(v));
