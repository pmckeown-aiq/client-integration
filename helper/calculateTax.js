...
module.exports = calculateTax = function(opts, feedTransactionArray, cb) {
  ...
  /* Tax Code Check ... 
   * We may have supplied a tax code on the feed, or we may have supplied to the lines by defaulting in from StockItemID ... We may even have tried to supply if from the AccountID on the header ... but the header version does not populate through to the lines (when i set defaults I do "header defaults" with "header values" and "line for line"
   * And we may have the tax code coming from the AccountID (Sales or Purchase). That needs to be grabbed as it would be for data validation (via the extractStaticData files
  ...
  if ( isTaxCodeSupplied.length >= 1 ) {
    ...
    if ( isUseAccountTaxCode.length >= 1 ) {
	  ...
	  if ( myCoIDObject.length === 1 ) {
        ...
        // Get the account tax code ... need to use the AccountID (or PurchaseAccountID)
        if ( typeof transaction.PurchaseAccountID !== 'undefined' ) {
	  var validateWhat = 'PurchaseAccountID';
	} else {
	  var validateWhat = 'AccountID';
	}
	...
	// ValidateWhat should be AccountID or PurchaseAccountID
