...
processData.prototype.CorlessInvoices = function(feedTransactions, opts, cb) {
  ...
  uniqueHeaders.forEach(function(header) {
    ...
    invLines.forEach(function(line) {
      ...
      if ( line.Newprice != '' ||  line.Newprice != 0 ) {
    ...
    vatStdLines.forEach(function(v){ 
    ...
    console.log(invoice.ExternalReference + ' has VAT lines ' + JSON.stringify(vatStdLines));
    vatReducedLines.forEach(function(v){ 
      // Start of with the status of the transaction line as true (it is a good transaction line)
      // tidy up vat lines
      delete v.StockItemDescription;
      // Set the StockItemPrice (the LineAmount) to be the TaxAmount
    ...
    console.log(invoice.ExternalReference + ' has Reduced VAT lines ' + JSON.stringify(vatReducedLines));
	...
	console.log('Line before discount ' + JSON.stringify(line));
	console.log('discount lines ' + JSON.stringify(discountLines));
        ...
        // Remove the applied discount line from the discountLines array (should end up with none at the end!) 
        _.remove(discountLines, { ParentID : line.ReservedResourceID });
        ...
        // START Check VAT
        var vatLines = _.merge(vatStdLines, vatReducedLines);
        var isMatch =  _.filter(vatLines, { ExpectedNetAmount : line.NetAmount });
        if (isMatch.length=== 1) {
          console.log(' WE HAVE A MATCH FOR 20% VAT ' + line.NetAmount + ':' + JSON.stringify(isMatch));
          console.log('isMatch Tax Amount ' + isMatch[0].TaxAmount);
          console.log('isMatch GL Code ' + isMatch[0].VatGLAccountCode);
          line.TaxAmount = isMatch[0].TaxAmount;
          // Now try to remove the matched element from the vatLines array
          _.reject(vatLines, { ExpectedNetAmount: line.NetAmount });
          ...
          if (isMatchMissed.length === 1) {
            ...
            if (isMatchVATAccumulated.length === 1) {
              ...
              var isAccumulatedNetAmounts =  _.reject(invoice.lines, function(nonVatLine) { 
              ...
              if ( totalTax.toFixed(2)/1 == isMatchVATAccumulated[0].TaxAmount ) {
                ...
                line.TaxAmount = line.NetAmount * 0.2 ;
                line.isWarning = { 'check': true, 'reason':'Accumulated VAT line found and simply set VAT to 20% of the Net Amount' }
              } else { // reject the invoice 
                     line.updateStatus = { 'status': false, 'error':'Some missing VAT on extra items - have VAT amount ' + isMatchVATAccumulated[0].TaxAmount + ' but expected ' + totalTax.toFixed(2)/1 };
              }
            }
          } else if ( line.GLAccountCode == "4004" ) {
            ...
            if ( vatLines.length === 1 ) { 
              line.updateStatus = { 'status': false, 'error':'Split Rate VAT - Please enter the invoice manually. One VAT Amount for ' + vatLines[0].TaxAmount + ' found. If all 20% should have been ' + (line.NetAmount * 0.2) };
            } else {
              line.updateStatus = { 'status': false, 'error':'Split Rate VAT - Please enter the invoice manually' };
            }
          } else {
            line.updateStatus = { 'status': false, 'error':'No VAT Found' };
          }
        } else {
          //console.log(' WE HAVE A SERIOUS ISSUE - MATCHED MORE THAN ONE LINE: ' + isMatch.length + ' LINES FOUND');
          line.updateStatus = { 'status': false, 'error':'Multiple VAT Found' }
        }
        ...
        //line.StockItemPrice = line.StockItemPrice.toFixed(2);
        // Some defaults
        var rate = 0.20;
        //line.TaxCode = rate;
        ...
        if (line.hasOwnProperty('updateStatus')) {
      ...
      invoice.lineCount = invoice.lines.length;
